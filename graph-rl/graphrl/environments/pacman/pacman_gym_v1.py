...
class EnlargeEnv(gym.ObservationWrapper):
    def __init__(self, env):
        super(EnlargeEnv, self).__init__(env)
        self.observation_space = gym.spaces.Box(low=0, high=255, dtype=np.uint8, shape=env.observation_space.shape)

    def observation(self, obs):

        new_obs = np.zeros((obs.shape[0] * 2, obs.shape[1] * 2), dtype=obs.dtype)
        for i in range(obs.shape[0]):
            for j in range(obs.shape[1]):
                val = obs[i, j]
                new_obs[i * 2:(i + 1) * 2, j * 2:(j + 1)*2] = val
        return new_obs
