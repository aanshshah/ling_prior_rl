...
from plot_utils import load_series
...
def main():
    ...
    parser.add_argument('--base_logdir', type=str, default='/localdisk')
    ...
    pdf_filename = 'plots_{}.pdf'.format(args.id)
    pdf_pages = PdfPages(pdf_filename)

    folder = os.path.join(args.base_logdir, 'log_{}'.format(args.id))
    reward_mean_files = glob.glob(os.path.join(folder, '*_episode_reward_mean_byepisode.txt'))

    all_reward_series = {}
    for filename in reward_mean_files:
        lastname = os.path.basename(os.path.normpath(filename))
        series_name = lastname.split('_episode_reward')[0]
        series = load_series(filename)
        episodes, values = zip(*series)
