...
class MalmoEnvSpecial(gym.Env):
    ...
    def obs_to_vector(self,observation,use_entities=True,flatten=True,expand_dims=True,add_inv=True,add_hotbar=True,add_goal=True):
        ...
        if use_entities: 
            ...
            if add_inv:
                entity_data[0][entity_data.shape[1] // 2][entity_data.shape[2] // 2] = self.add_selected_inventory(observation)
        ...
        if add_hotbar:
            state_data = np.concatenate((state_data,self.get_hotbar(observation)),axis=1)
    ...
    def add_selected_inventory(self,observation):
        # print(observation)
        selected = observation['currentItemIndex']
        key = 'InventorySlot_{}_item'.format(selected)
        if observation[key] in self.relevant_entities:
            return self.entity_map[observation[key] ]
        else: return 0

    def get_hotbar(self,observation):
        # print(observation)
        hot_bar = np.zeros((1,1,10))
        for i in range(10):
           key = 'InventorySlot_{}_item'.format(i)
           hot_bar[0][0][i] = self.entity_map[observation[key] ] if observation[key] in self.relevant_entities else 0
        return hot_bar
    ...
    def load_mission_param(self,mission_type):
        ...
        # kg_entities = [env_to_kg_entity[entity] if entity in env_to_kg_entity else entity for entity in entities]
        # kg_to_env_entity = {}
    ...
    def __init__(self,mission_type,port):
     ...
     #   print(mission_param)
        self.actions =["movenorth","movesouth", "movewest", "moveeast","attack","use","hotbar.1", "hotbar.2", "hotbar.3", "hotbar.4", "hotbar.5", "hotbar.6",
                     "hotbar.7", "hotbar.8", "hotbar.9"] #,"strafe 1","strafe -1"] #,"attack 1","attack 0"]
