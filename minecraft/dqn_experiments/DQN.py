...
class DQNSolver:
    ...
    def experience_replay(self):
          ...
          with tf.GradientTape() as tape:
              ...
              QA_values = tf.gather(tf.reshape(q_values,[-1]),tf.convert_to_tensor(self.action_space*np.arange(len(actions))+actions, dtype=tf.int32))

              next_QA_values = np.max(q_tilde,axis=-1)
              discounted_rewards =np.array(next_rewards,dtype=np.float64)+ self.gamma * next_QA_values
              discounted_rewards = tf.convert_to_tensor(discounted_rewards,dtype=tf.float64)
